...
"""
Model Definition for ACTPC-applied to Patient data.

Date Last updated: 07 June 2021
...
from tensorflow.keras.layers import LSTM, Masking, Dense, Attention
...
class Static_Encoder(layers.Layer):
    ...
    output tensors expected of shape (bs, latent dim).

    Encoder Layer stacks LSTMs layers using  whole output sequences - the overall output is of size (bs, latent dim),
    with [i, :] representing the latent vector for sample i.
    ...
    def build(self, input_shape = None):
        ...
        self.mask_layer = Masking(mask_value = self.mask_value)
        ...
        self.output_layer     = LSTM(units = self.latent_dim, activation = self.state_fn,
                recurrent_activation = self.recurrent_fn, return_sequences = False, dropout = self.dropout,
...
class Time_Encoder(layers.Layer):
    """
    Class for an Encoder layer architecture. Input tensors expected to be of shape (bs x times x xdim), and
    output tensors expected of shape (bs, times, latent dim).

    Encoder Layer stacks LSTMs layers using  whole output sequences - the overall output is of size (bs, latent dim),
    with [i, , j:] representing the latent vector for sample i.

    Params:

    - latent_dim          : dimensionality of latent space for the time-series. (default = 1)
    - hidden_layers       : Number of "hidden" LSTM layers. Total number of stacked layers is hidden_layers + 1 (default = 1)
    - state_fn            : The activation function to use on cell state/output. (default = 'tanh')
    - recurrent_activation: The activation function to use on forget/input/output gates. (default = 'sigmoid')
    - dropout             : dropout rate to be used on cell state/output computation. (default = 0.6)
    ...
    def __init__(self, intermediate_dim=32, latent_dim = 1, hidden_layers=1, hidden_nodes=20, state_fn="tanh", recurrent_fn="sigmoid",
                 dropout=0.7, recurrent_dropout=0.0, mask_value=0.0, name='encoder'):
