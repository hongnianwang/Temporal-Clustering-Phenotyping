...
class Attention_Encoder(layers.Layer):
    
    """
    Class for an Attention_Encoder layer architecture. Input tensors expected to be of shape (bs x times x xdim), and
    output tensors expected of shape (bs x times x latent dim).

    Attention_Encoder Layer stacks LSTMs layers using  whole output sequences - the overall output is of size (bs x time_steps x latent dim), with [i, j, :] representing the latent vector from time-subsequence corresponding to patient i and from times t=0, to t=j.
    ...
    - name                : Name on which to save component. (default = 'attention_encoder')
    ...
    def __init__(self, intermediate_dim=32, hidden_layers=1, hidden_nodes=20, state_fn="tanh", recurrent_fn="sigmoid",
                 dropout=0.7, recurrent_dropout=0, mask_value=0.0, name='attention_encoder'):
    ...
    def build(self, input_shape = None):
        ...
        self.output_layer     = LSTM(units = self.intermediate_dim, activation = self.state_fn,
                ...
                recurrent_dropout = self.recurrent_dropout, return_state = True, name = self.name)
...
class ACTPC(tf.keras.Model):
    ...
    def __init__(self, num_clusters = 12, output_dim = 4, y_type = "categorical",
         latent_dim = 32, beta = 0.01, alpha = 0.01, seed = 4347, embeddings = None, num_encoder_layers = 0, num_encoder_nodes = 20, state_fn = 'tanh', recurrent_activation = 'sigmoid', encoder_dropout = 0.6, recurrent_dropout = 0, mask_value = 0.0,  encoder_name = 'attention_encoder', selector_name = 'selector', num_selector_layers = 1, num_selector_nodes = 20, selector_fn = 'sigmoid', selector_output_fn = 'softmax', selector_dropout = 0.6, predictor_name = 'predictor', num_predictor_layers = 1, num_predictor_nodes = 20, predictor_fn = 'sigmoid', predictor_dropout = 0.6):
    ...
    def build(self, input_shape = None):
        ...
        self.Encoder           = Attention_Encoder(intermediate_dim = self.latent_dim,
    ...
    def train_step(self, inputs, training = True):
        ...
        with tf.GradientTape(watch_accessed_variables=False) as tape:
            ...
            # Output vector given sampled cluster embedding
            y_pred       = self.Predictor(cluster_sel, training = training)    
        ...
        return {'L1': L1_cri.result(), 'L2': L1_act.result(), 'L3': L_emb.result()}
    ...
    def test_step(self, inputs):
        ...
        clus_sep_loss    = net_utils.euclidean_separation_loss(y_clusters = tf.expand_dims(self.embeddings, axis = 0))
        ...
        return {'L1': val_L1_cri.result(), 'L2': val_L1_act.result(),
                'L3': val_L_emb.result(), 'pis': val_L2.result(), 'Sep': L3.result(),
                'KL':  KL_sep_loss, "AUC": auroc.result(), "ACC": cat_acc.result()} 
