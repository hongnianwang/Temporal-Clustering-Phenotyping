...
def predictive_clustering_loss(y_true, y_pred, y_type = 'categorical', name = 'pred_clus_L'):
    ...
    if y_type == 'binary':
        ...
        batch_loss = -tf.reduce_mean(tf.reduce_sum(y_true * log(y_pred + 0.5), axis = -1), name = name)
...
def euclidean_separation_loss(y_clusters, name = 'emb_sep_L'):
    ...
    y_clusters: shape (1, num_clusters, output_dim) - for each cluster i, y_embeddings[i] is the
    ...
    # Compute L1 distance
    pairwise_loss = math.reduce_euclidean_norm(squared_difference(embedding_column, embedding_row),
                                  axis = -1)  # shape 1, K, K
...
def get_callbacks(model_name, folder, loss_names, metric_names, track_metric, csv_log = True, early_stop = True, 
    ...
    save_folder = folder + 'experiments/attention/{}/{}/checkpoints/'.format(today, model_name)
    csv_folder  = folder + 'experiments/attention/{}/{}/results/'.format(today, model_name)
